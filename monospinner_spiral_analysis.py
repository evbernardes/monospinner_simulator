...
subplot(l,c,3,1)
plt.plot(leg, bb)
#plt.plot(leg, np.log(bb - min(bb) ))
#plt.plot(leg, bb)
Pb = Polynomial.fit(leg, bb, 6)
Sfit = fit_sin(leg, bb)['fitfunc']

#plt.plot(leg, Pb(leg), 'k', linestyle = 'dashed')
#plt.plot(leg, Sfit(leg), 'r', linestyle = 'dashed')
...
#plt.plot(leg[10:], Pd(leg[10:]), 'k', linestyle = 'dashed')
plt.xlabel(r'$\Delta \theta$ (spin drift)')
plt.ylabel(r'$d$')
...
#plt.plot(leg, bb / dd)
plt.plot(leg, dd / bb)
#plt.plot(leg, diff(bb / dd)/diff(leg))
